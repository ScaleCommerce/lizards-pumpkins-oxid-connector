...
class oxSeoEncoder extends oxSuperCfg
{
    ...
    public function addLanguageParam($sSeoUrl, $iLang)
    {
        ...
        if ($iLang != $iDefLang &&
            ...
            getStr()->strpos($sSeoUrl, $this->_replaceSpecialChars($aLangIds[$iLang]) . '/') !== 0
        ) {
    ...
    protected function _prepareUri($sUri, $iLang = false)
    {
        ...
        $sUri = $this->_replaceSpecialChars($sUri);
     ...
     *
     * @param string $sStringWithSpecialChars
     *
     * @return string
     */
    protected function _replaceSpecialChars($sStringWithSpecialChars)
    {
        if (!is_string($sStringWithSpecialChars)) {
            return "";
        }
        $oStr = oxStr::getStr();
        // #0004282 bugfix, php <5.3 does not escape - char, so we do it manually
